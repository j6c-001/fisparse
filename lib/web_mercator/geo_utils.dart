...
Vector2 destination(double lng, double lat, {@required num distance, num bearing = 0}) {
  assert(distance.isFinite && distance > 0, 'distance is invalid');

  final lng1 = lng * degrees2Radians;
  final lat1 = lat * degrees2Radians;
  final bearingRad = bearing * degrees2Radians;
  final radians = lengthToRadians(distance);

  final lat2 = asin(sin(lat1) * cos(radians) + cos(lat1) * sin(radians) * cos(bearingRad));
  final lng2 = lng1 + atan2(sin(bearingRad) * sin(radians) * cos(lat1), cos(radians) - sin(lat1) * sin(lat2));
